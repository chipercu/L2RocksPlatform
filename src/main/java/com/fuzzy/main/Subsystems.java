...
public class Subsystems implements AutoCloseable {
    ...
    protected Subsystems(Builder builder) throws Exception {
        ...
        if (networkConfig != null) {
            GrpcNetworkTransit.Builder networkTransitBuilder = new GrpcNetworkTransit.Builder(uncaughtExceptionHandler);
            if (networkConfig.getName() != null) {
                networkTransitBuilder.withNodeName(networkConfig.getName());
            }
            if (networkConfig.getPort() != null) {
                networkTransitBuilder.withServer(new GrpcNetworkTransit.Builder.Server(networkConfig.getPort()));
            }
    ...
    public void checkBeforeUpgrade() throws Exception {
        log.info("Subsystems check before upgrade...");
        platform.checkBeforeUpgrade();
        log.info("Subsystems check before upgrade... complete");
    }
