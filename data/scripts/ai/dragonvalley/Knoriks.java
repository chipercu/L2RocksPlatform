...
{
	private static int KNORIKS_ACTIVATE_SKILL_CHANGE 	= 5;
	private static int KNORIKS_SEARCH_RADIUS		 	= 600;
	private static int KNORIKS_SKILL_DBUFF_ID			= 6744;

	public Knoriks(L2NpcInstance actor)
	{
		super(actor);
	...
	public void ATTACKED(L2Character attacker, int damage, L2Skill skill)
	{
		...
		if(!attacker.isPlayer() || actor.isDead() || actor.isOutOfControl() || actor.isActionsDisabled() || !Rnd.chance(KNORIKS_ACTIVATE_SKILL_CHANGE))
			return;
		else if(attacker != null)
			actor.doCast(SkillTable.getInstance().getInfo(KNORIKS_SKILL_DBUFF_ID, 1), (L2Player)attacker, false);	
	}
}
