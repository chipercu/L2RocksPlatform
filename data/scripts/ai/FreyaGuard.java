 ...
 * 18854, 18855, 18856
 **/
public class FreyaGuard extends Fighter
{
	public FreyaGuard(L2Character actor)
	{
		super(actor);
	}
	...
	protected void tryMoveToTarget(L2Character target, int range)
	{
		...
		if(_pathfind_fails >= 10 /*&& System.currentTimeMillis() - (actor.getAttackTimeout() - getMaxAttackTimeout()) < getTeleportTimeout() && actor.isInRange(target, 2000)*/)
		{
			...
			{
				//_log.info("FreyaGuard: returnHome");
				//returnHome(true);
				//return false;
			}
			try
			{
				...
				if(size > 0)
				{
					...
					if(!playerAtack(player, size, 0))
						for(int i=0;i<size;i++)
							if(playerAtack(actor.getReflection().getPlayers().get(i), size, i))
								break;
				}
			}
	...
	protected void onEvtSpawn()
	{
		super.onEvtSpawn();
		/*L2NpcInstance actor = getActor();
		for(L2Player p : actor.getReflection().getPlayers())
			if(playerAtack(player, 0, 0))
			{
				notifyEvent(CtrlEvent.EVT_AGGRESSION, p, 5000);
				p.addDamageHate(actor, 0, 1000);
			}*/
	}
	...
	protected boolean thinkActive()
	{
		...
		if(actor.getReflectionId() < 1)
			actor.deleteMe();
		if(actor.getNpcId() == 18854 || actor.getNpcId() == 18855 || actor.getNpcId() == 18856)
		{
			try
			{
				int size = actor.getReflection().getPlayers().size();
	...
	private boolean playerAtack(L2Player player, int size, int count)
	{
		L2NpcInstance actor = getActor();
		if(player != null)
		{
			if(player.isDead() || player.isInvisible() && player.isGM() || player.isInZone(L2Zone.ZoneType.other)/* || (!GeoEngine.canAttacTarget(actor, player, false) && size-count > 5)*/)
			//if(player.isDead() || !GeoEngine.canAttacTarget(actor, player, false))
				return false;
