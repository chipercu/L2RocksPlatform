package quests._738_DestroyKeyTargets;

import l2open.gameserver.model.base.ClassId;
import quests.Dominion_KillSpecialUnitQuest;

/**
 * @author VISTALL
 * @date 16:23/12.04.2011
 */
public class _738_DestroyKeyTargets extends Dominion_KillSpecialUnitQuest
{
	public _738_DestroyKeyTargets()
	{
		super(738);
	}

	@Override
	protected int startNpcString()
	{
		return 73851; // DEFEAT_S1_WARSMITHS_AND_OVERLORDS;
	}

	@Override
	protected int progressNpcString()
	{
		return 73861; // YOU_HAVE_DEFEATED_S2_OF_S1_WARSMITHS_AND_OVERLORDS;
	}

	@Override
	protected int doneNpcString()
	{
		return 73862; // YOU_DESTROYED_THE_ENEMYS_PROFESSIONALS;
	}

	@Override
	protected int getRandomMin()
	{
		return 3;
	}

	@Override
	protected int getRandomMax()
	{
		return 8;
	}

	@Override
	protected ClassId[] getTargetClassIds()
	{
		return new ClassId[]{
				ClassId.overlord,
				ClassId.warsmith,
				ClassId.dominator,
				ClassId.maestro
		};
	}
}
