package quests._10294_SevenSignsMonasteryofSilence;
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("readingdesk_q10294_greentrue2.htm"))
		{
			if(st.getInt("green") == 0)
			{
				npc.setNpcState(1);
				st.set("green", 1);
				NpcUtils.spawnSingle(JudevanEtinasEvilThoughts, new Location(87704, -249496, -8320, 49152), st.getPlayer().getReflection().getId(), 0);
				for(int i = 0; i < 3; i++)
					NpcUtils.spawnSingle(SolinaLayrother, Location.findPointToStay(st.getPlayer(), 300), st.getPlayer().getReflection().getId(), 0);
				proccessComplete(st);
			}
			else
				htmltext = "readingdesk_q10294_0.htm";
		...
		else if(event.equalsIgnoreCase("readingdesk_q10294_bluetrue2.htm"))
		{
			if(st.getInt("blue") == 0)
			{
				npc.setNpcState(1);
				st.set("blue", 1);
				NpcUtils.spawnSingle(SolinasEvilThoughts, new Location(86680, -246728, -8320, 0), st.getPlayer().getReflection().getId(), 0);
				proccessComplete(st);
			}
			else
				htmltext = "readingdesk_q10294_0.htm";
		...
		else if(event.equalsIgnoreCase("readingdesk_q10294_redtrue2.htm"))
		{
			if(st.getInt("red") == 0)
			{
				npc.setNpcState(1);
				st.set("red", 1);
				NpcUtils.spawnSingle(JudevanEtinasEvilThoughts2, new Location(84840, -252392, -8320, 49152), st.getPlayer().getReflection().getId(), 0);
				proccessComplete(st);
			}
			else
				htmltext = "readingdesk_q10294_0.htm";
