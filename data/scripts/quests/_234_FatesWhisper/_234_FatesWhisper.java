...
public class _234_FatesWhisper extends Quest implements ScriptFile
{
	...
	private final static int PLATE_TRIBE[] = { 20823, 20826, 20827, 20828, 20829 };
	...
	private static List<String> _hwid = new ArrayList<String>();
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("31002-08a.htm"))
		{
			...
			return ConfigValue.AltAllowSubClassWithoutBaium ? "30833-01a.htm" : "30833-01n.htm";
		...
		if(newweapon != 0)
		{
			...
			if(itemtotake != null)
			{
				...
				htmltext = "make.htm";
				st.exitCurrentQuest(ConfigValue._234_FatesWhisper);
			}
			else
				htmltext = "noweapon.htm";
		}
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond == 0)
				if(st.getPlayer().getLevel() >= ConfigValue.NoblessSellSubLevel)
		...
		{
			if(ConfigValue.CabrioHwidProtect && !check(st.getPlayer()))
			{
				htmltext = "31027-hwid.htm";
			}
			else
			{
				if(st.getPlayer().getVarB("buy_quest_subclass", false))
				{
					Quest q = QuestManager.getQuest("_234_FatesWhisper");
					QuestState qs = st.getPlayer().getQuestState(q.getClass());
					if(qs != null)
						qs.exitCurrentQuest(true);
					q.newQuestState(st.getPlayer(), Quest.COMPLETED);

					if(st.getPlayer().getRace() == Race.kamael)
					{
						q = QuestManager.getQuest("_236_SeedsOfChaos");
						qs = st.getPlayer().getQuestState(q.getClass());
						if(qs != null)
							qs.exitCurrentQuest(true);
						q.newQuestState(st.getPlayer(), Quest.COMPLETED);
					}
					...
					NpcHtmlMessage html = new NpcHtmlMessage(st.getPlayer(), npc);
					html.setFile("data/html/cabrio.htm");
					st.getPlayer().sendPacket(html);
					return null;
				}
				else
				{
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if(cond == COND1 && npcId == SHILLEN_MESSAGER)
		{
			...
			if(isQuest == null)
			{
				...
				st.startQuestTimer("CABRIOCOFFER_Fail", ConfigValue.CabrioCofferDespawn * 1000);
			}
		}
		...
		if(cond == COND8 && (npcId == GUARDIAN_ANGEL || npcId == SEAL_ANGEL || arrayContains(PLATE_TRIBE, npcId)))
		{
