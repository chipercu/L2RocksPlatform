...
public class _307_ControlDeviceOfTheGiants extends Quest implements ScriptFile
{
	...
	public _307_ControlDeviceOfTheGiants()
	{
		...
		addStartNpc(Droph);
		addTalkId(Droph);
		addKillId(HekatonPrime, GORGOLOS, LAST_TITAN_UTENUS, GIANT_MARPANAK);
		addQuestItem(CaveExplorationText1Sheet, CaveExplorationText2Sheet, CaveExplorationText3Sheet);
	}
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("summon_rb"))
		{
			if(ServerVariables.getLong("HekatonPrimeRespawn", 0) < System.currentTimeMillis() && st.getQuestItemsCount(CaveExplorationText1Sheet) >= 1 && st.getQuestItemsCount(CaveExplorationText2Sheet) >= 1 && st.getQuestItemsCount(CaveExplorationText3Sheet) >= 1)
			{
				...
				L2NpcInstance boss = st.addSpawn(HekatonPrime, HekatonPrimeLoc.x, HekatonPrimeLoc.y, HekatonPrimeLoc.z, HekatonPrimeLoc.h, 0, 0);
				if(((L2MonsterInstance)boss).getMinionList() == null)
					((L2MonsterInstance)boss).setNewMinionList();
				((L2MonsterInstance)boss).getMinionList().maintainMinions();
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		switch (npcId)
		{
			case GORGOLOS:
			{
				st.giveItems(CaveExplorationText1Sheet, 1);
				st.playSound(SOUND_ITEMGET);
				break;
			}
			case LAST_TITAN_UTENUS:
			{
				st.giveItems(CaveExplorationText2Sheet, 1);
				st.playSound(SOUND_ITEMGET);
				break;
			}
			case GIANT_MARPANAK:
			{
				st.giveItems(CaveExplorationText3Sheet, 1);
				st.playSound(SOUND_ITEMGET);
				break;
			}
			case HekatonPrime:
			{
				if(st.getPlayer().isInParty())
				{
					for(L2Player p : st.getPlayer().getParty().getPartyMembers())
					{
						final QuestState qs = p.getQuestState(getName());
