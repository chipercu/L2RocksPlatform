...
public class _503_PursuitClanAmbition extends Quest implements ScriptFile
{
	...
	public void giveItem(int item, long maxcount, QuestState st)
	{
		...
		if(leader == null)
			return;
		if(player.getDistance(leader) > ConfigValue.AltPartyDistributionRange)
			return;
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30762-03.htm"))
		{
			...
			st.addSpawn(BlitzWyrm, npc.getLoc().x, npc.getLoc().y, npc.getLoc().z, 312, 300, 120000);
			st.addSpawn(BlitzWyrm, npc.getLoc().x, npc.getLoc().y, npc.getLoc().z, 23241, 300, 120000);
		}
		// Events Fritz
		else if(event.equalsIgnoreCase("30761-03.htm"))
		{
			...
			st.addSpawn(BlitzWyrm, npc.getLoc().x, npc.getLoc().y, npc.getLoc().z, 2342, 300, 120000);
			st.addSpawn(BlitzWyrm, npc.getLoc().x, npc.getLoc().y, npc.getLoc().z, 1123, 300, 120000);
		}
		// Events Kusto
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		switch(cond)
		{
			case 2:
				switch(npcId)
				{
					case ThunderWyrm1: // 20282
						if(Rnd.chance(20))
							giveItem(Th_Wyrm_Eggs, 10, st);
						break;

					case ThunderWyrm2: // 20243
						if(Rnd.chance(15))
							giveItem(Th_Wyrm_Eggs, 10, st);
						break;
					case Drake1: // 20137
