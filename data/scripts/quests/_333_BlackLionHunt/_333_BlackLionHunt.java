...
public class _333_BlackLionHunt extends Quest implements ScriptFile
{
	...
	public void giveRewards(QuestState st, int item, long count)
	{
		...
		if(count >= 20)
			st.giveItems(LIONS_CLAW, (count / 20) * (long) st.getRateQuestsRewardDrop());
	}
		...
		else if(event.equalsIgnoreCase("continue"))
		{
			...
			if(claw > 0)
			{
				...
				while(claw > 0)
				{
					int n = Rnd.get(5);
					if(n == 0)
						st.giveItems(ALACRITY_POTION, Math.round(ala_count * st.getRateQuestsRewardDrop()));
					else if(n == 1)
						st.giveItems(SOULSHOT_D, Math.round(soul_count * st.getRateQuestsRewardDrop()));
					else if(n == 2)
						st.giveItems(SCROLL_ESCAPE, Math.round(soe_count * st.getRateQuestsRewardDrop()));
					else if(n == 3)
						st.giveItems(SPIRITSHOT_D, Math.round(spir_count * st.getRateQuestsRewardDrop()));
					else if(n == 4)
						st.giveItems(HEALING_POTION, Math.round(heal_count * st.getRateQuestsRewardDrop()));
					claw -= 1;
				}
				if(check_eye > 0)
					return "30735-06.htm";
