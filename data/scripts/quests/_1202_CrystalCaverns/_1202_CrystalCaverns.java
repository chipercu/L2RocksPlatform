...
public class _1202_CrystalCaverns extends Quest implements ScriptFile
{
	...
	public void onLoad()
	{
		ScriptFile._log.info("Loaded Quest: 1202: Crystal Caverns");
	}
	...
	public String onFirstTalk(L2NpcInstance npc, L2Player player)
	{
		...
		if(spawn_captain && Rnd.chance(50))
		{
			...
			captain.addStatFunc(new FuncMul(Stats.p_physical_attack, 0x30, this, 5));
			captain.addStatFunc(new FuncMul(Stats.p_magical_attack, 0x30, this, 5));
			captain.addStatFunc(new FuncMul(Stats.p_physical_defence, 0x30, this, 5));
			captain.addStatFunc(new FuncMul(Stats.p_magical_defence, 0x30, this, 5));
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("meet"))
		{
			...
			if(player.isInParty())
			{
				for(L2Player c0 : player.getParty().getPartyMembers())
				{
					if(c0 != null)
					{
						if(c0.getReflectionId() == npc.getReflectionId())
						{
							if(npc.DistFromMe(c0) <= 1000)
							{
								if(npc.OwnItemCount(c0,9695) < 1)
								{
									npc.Shout(npc.MakeFString(1800028,c0.getName(),"","","",""));
									return "meeting.htm";
								}
								if(npc.OwnItemCount(c0,9696) < 1)
								{
									npc.Shout(npc.MakeFString(1800027,c0.getName(),"","","",""));
									return "meeting.htm";
								}
								if(npc.OwnItemCount(c0,9697) < 1)
								{
									npc.Shout(npc.MakeFString(1800029,c0.getName(),"","","",""));
									return "meeting.htm";
								}
							}
							else
							{
								npc.Shout(npc.MakeFString(1800030,c0.getName(),"","","",""));
								return "meeting.htm";
							}
						}
					}
				}
				BaylorManager.entryToBaylorLair(player, npc);
	...
	public String onKill(L2NpcInstance npc, QuestState st)
	{
		...
		long seedsCount = (long) (1 * ConfigValue.RateDropItems);
		if(npcId == DARNEL)
		{
			// 32276
