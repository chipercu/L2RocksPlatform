...
public class _642_APowerfulPrimevalCreature extends Quest implements ScriptFile
{
	...
	private static int[] Rewards2 = { 9967,9968,9969,9970,9971,9972,9973,9974,9975, 10545, 10544 };
	// Quest Items
	...
	private static int Dinosaur_Egg_Chance = 100;
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("dindin_q0642_12.htm") && _state == STARTED)
		{
		...
		else if(event.equalsIgnoreCase("dindin_q0642_10.htm") && _state == STARTED && ConfigValue.Alt100PercentRecipesA)
			...
			try
			{
				...
				for(int reward : Rewards)
					if(reward == rew_id && Dinosaur_Tissue_Count >= 150 && st.getQuestItemsCount(Dinosaur_Egg) >= 1)
					{
						st.takeItems(Dinosaur_Tissue, 150);
						st.takeItems(Dinosaur_Egg, 1);
						st.giveItems(ConfigValue.Alt100PercentRecipesA ? reward + 1 : reward, 1, false);
				...
				for(int reward : Rewards2)
					if(reward == rew_id && Dinosaur_Tissue_Count >= 450)
					{
