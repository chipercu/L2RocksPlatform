...
public class _061_LawEnforcement extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, L2NpcInstance npc)
	{
		...
		{
			st.giveItems(ADENA_ID, 26000, ConfigValue.RateQuestsRewardOccupationChange);
			st.getPlayer().setClassId(ClassId.judicator.ordinal());
			st.getPlayer().broadcastUserInfo(true);
			st.getPlayer().broadcastSkill(new MagicSkillUse(st.getPlayer(), 4339, 1, 6000, 1), true);
			st.getPlayer().broadcastSkill(new MagicSkillUse(npc, 4339, 1, 6000, 1), true);
			st.exitCurrentQuest(true);
		}
		return htmltext;
	}
