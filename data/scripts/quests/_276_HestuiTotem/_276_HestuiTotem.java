	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		if(_state == CREATED)
		{
			...
			if(st.getQuestItemsCount(Kasha_Crystal) > 0)
			{
				...
				if(st.getRateQuestsRewardAdena() > 1)
					st.giveItems(57, Math.round(ItemTemplates.getInstance().getTemplate(Totem_of_Hestui).getReferencePrice() * (st.getRateQuestsRewardAdena() - 1) / 2), false);
