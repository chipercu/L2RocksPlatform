...
public class SimpleLottery extends BaseBBSManager implements ICommunityHandler, ScriptFile
{
	...
	public void onLoad()
	{
		...
		_log.info("CommunityBoard: Simple Lottery games loaded.");
		CommunityHandler.getInstance().registerCommunityHandler(this);
		restoreLotteryData();
		_log.info("CommunityBoard: Simple Lottery games played " + Util.formatAdena(getTotalGames()) + ".");
		restoreJackpot();
		_log.info("CommunityBoard: Simple Lottery jackpot is " + Util.formatAdena(jackpot) + " " + DifferentMethods.getItemName(ConfigValue.SLotteryItem) + ".");
		restoreWinnerData();
		restoreStats();
	}
	...
	public void parsecmd(String bypass, L2Player player)
	{
		...
		String html = Files.read(ConfigValue.CommunityBoardHtmlRoot + "cabinet/games/simple_lottery.htm", player);

		if(!ConfigValue.SLotteryAllow)
		{
			player.sendMessage(new CustomMessage("scripts.services.off", player));
			return;
		}
		else if(bypass.startsWith(Commands._bbsslottery.toString()))
		{
			boolean win = false;
			boolean index = false;
			...
			if(task[1].equals("play"))
			{
				if(check(player))
				{
					int price = ConfigValue.SLotteryBet[Integer.parseInt(task[2])];
					if(DifferentMethods.getPay(player, ConfigValue.SLotteryItem, price, true))
					{
						UpdateGames();
						if(Rnd.chance(ConfigValue.SLotteryWinChance))
						{
							wins++;
							win_count = win_count + (price * ConfigValue.SLotteryRewardMul);
							win = true;
							Functions.addItem(player, ConfigValue.SLotteryItem, price * ConfigValue.SLotteryRewardMul);
							if(Rnd.chance(ConfigValue.SLotteryJackpotChance))
							{
								win_jackpot = true;
								jackpot_win++;
								jackpot_count = jackpot_count + jackpot;
								Functions.addItem(player, ConfigValue.SLotteryItem, jackpot);
								...
								String[] param = {
										...
										String.valueOf(DifferentMethods.getItemName(ConfigValue.SLotteryItem)) };
								...
								Log.add(" " + player.getName() + " win jackpot " + jackpot + " " + DifferentMethods.getItemName(ConfigValue.SLotteryItem) + "", "SimpleLottery");
								nulledJackpot();
							}
						}
						else
						{
							...
							setJackpot(price * ConfigValue.SLotteryToJacktop / 100);
						}
					}
				}
			}
			...
			else if(task[1].equals("simple_stats"))
			{
				index = true;
				stats_page = true;
			...
			html = html.replace("<?lottery_jackpot?>", win_jackpot ? new CustomMessage("communityboard.games.lottery.jackpot.win", player).toString() : Util.formatAdena(jackpot));
			...
			html = html.replace("<?lottery_item?>", String.valueOf(ConfigValue.SLotteryItem));
			html = html.replace("<?lottery_max_jackpot?>", String.valueOf(Util.formatAdena(ConfigValue.SLotteryMaxJacktop)));
			html = html.replace("<?lottery_to_jackpot>", String.valueOf(ConfigValue.SLotteryToJacktop));
			html = html.replace("<?lottery_win_mul>", String.valueOf(ConfigValue.SLotteryRewardMul));
			html = html.replace("<?stats?>", player.isGM() ? "<font color=\"FF0000\"><a action=\"bypass -h _bbsslottery:simple_stats\">Статистика</a></font>" : "<br>");
	...
	{
		if(player.getLevel() < ConfigValue.SLotteryLevel[0])
		{
			player.sendMessage(new CustomMessage("common.level.is.low", player).addNumber(ConfigValue.SLotteryLevel[0]));
			return false;
		}
		else if(player.getLevel() > ConfigValue.SLotteryLevel[1])
		{
			player.sendMessage(new CustomMessage("common.level.is.high", player).addNumber(ConfigValue.SLotteryLevel[1]));
			return false;
		}
		else
			return true;
	...
	public static String button(L2Player player)
	{
		...
		int block = ConfigValue.SLotteryBet.length / 2;
		for(int i = 1; i <= ConfigValue.SLotteryBet.length; i++)
		{
			html.append("<td>");
			html.append("<button action=\"bypass -h _bbsslottery:play:" + (i - 1) + "\" value=\"" + new CustomMessage("communityboard.games.lottery.bet", player).addString(Util.formatAdena(ConfigValue.SLotteryBet[i - 1])).toString() + "\" width=200 height=31 back=\"L2UI_CT1.OlympiadWnd_DF_Watch_Down\" fore=\"L2UI_CT1.OlympiadWnd_DF_Watch\">");
			html.append("</td>");
	...
	private String showWinnerPage(L2Player player)
	{
		...
		html.append(new CustomMessage("common.currency", player).addItemName(ConfigValue.SLotteryItem).toString());
		html.append("</td>");
		html.append("</tr>");
		html.append("</table>");
		html.append("</td>");
	...
	private String showStats()
	{
		...
		html.append("<br>");
		html.append("<font name=\"hs12\">Лотерея</font>");
	...
	private void restoreStats()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				if(rset.getString("type").equals("simple_wins"))
					wins = rset.getInt("count");
				else if(rset.getString("type").equals("simple_win_count"))
					win_count = rset.getInt("count");
				else if(rset.getString("type").equals("simple_loss"))
					loss = rset.getInt("count");
				else if(rset.getString("type").equals("simple_loss_count"))
					loss_count = rset.getInt("count");
				else if(rset.getString("type").equals("simple_jackpot_win"))
					jackpot_win = rset.getInt("count");
				else if(rset.getString("type").equals("simple_jackpot_count"))
					jackpot_count = rset.getInt("count");
			}
		}
		catch(SQLException e)
		{
			_log.warning("SimpleLottery: Could not restore lottery stats: " + e);
		}
		finally
		{
	...
	private void store()
	{
		...
		try
		{
			_log.info("SimpleLottery: Store Jackpot");
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + wins + " WHERE `type`='simple_wins'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + win_count + " WHERE `type`='simple_win_count'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + loss + " WHERE `type`='simple_loss'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + loss_count + " WHERE `type`='simple_loss_count'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + jackpot_win + " WHERE `type`='simple_jackpot_win'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + loss_count + " WHERE `type`='simple_loss_count'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + jackpot + " WHERE `type`='simple_jackpot'");
			statement.execute();
			statement = con.prepareStatement("UPDATE `bbs_lottery` SET `count`=" + getTotalGames() + " WHERE `type`='simple_total_games'");
			statement.execute();
		}
		catch(SQLException e)
		{
			_log.warning("SimpleLottery: Could not store lottery stats: " + e);
		}
		finally
		{
	...
	private void restoreWinnerData()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT * FROM `bbs_lottery` WHERE `type`='simple_winner' ORDER BY `count` DESC LIMIT 0,8");
			rset = statement.executeQuery();
		...
		{
			_log.warning("SimpleLottery: Could not restore lottery winner: " + e);
		}
		finally
		{
			DatabaseUtils.closeDatabaseCSR(con, statement, rset);
	...
	private boolean restoreJackpot()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT `count` FROM `bbs_lottery` WHERE `type`='simple_jackpot'");
			rset = statement.executeQuery();
		...
		{
			_log.warning("SimpleLottery: Could not restore lottery jackpot: " + e);
		}
		finally
		{
			DatabaseUtils.closeDatabaseCSR(con, statement, rset);
	...
	private boolean restoreLotteryData()
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT `count` FROM `bbs_lottery` WHERE `type`='simple_total_games'");
			rset = statement.executeQuery();
		...
		{
			_log.warning("SimpleLottery: Could not restore lottery games: " + e);
		}
		finally
		{
			DatabaseUtils.closeDatabaseCSR(con, statement, rset);
	...
	private void updateWinner(long count, String name)
	{
		...
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("INSERT INTO `bbs_lottery` (`count`, `type`, `name`) VALUES (" + count + ", 'simple_winner', '" + name + "');");
			statement.execute();
		}
		catch(SQLException e)
		{
			_log.warning("SimpleLottery: Could not increase current lottery winner: " + e);
		}
		finally
		{
			DatabaseUtils.closeDatabaseCS(con, statement);
	...
	private void setJackpot(long count)
	{
		jackpot = Math.min(jackpot + count, ConfigValue.SLotteryMaxJacktop);
	}

	private void nulledJackpot()
	{
		jackpot = ConfigValue.SLotteryNullJacktop;
	}
