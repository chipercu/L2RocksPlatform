...
{
	private static final String[][] HERO_CLOAK =
	{
		{
			"32799",
			"BranchSys2.Icon.br_freya_cloak_i00",
			"Adimus Cloak",
			"Cloak"
		},
		{
			"32800",
			"BranchSys2.Icon.7anni_cloack_i00",
			"Adoel Cloak",
			"Cloak"
		},
		{
			"32801",
			"BranchSys2.event_7anni.br_magic_7anni_cloak_red_t01",
			"Metatron Cloak",
			"Cloak"
		},
		{
			"32802",
			"BranchSys2.Icon.7anni_cloack_i02",
			"Seraph Cloak",
			"Cloak"
		}
	};
	...
	public void getweapon(String[] var)
	{
		...
		if(player.isHeroType() == 0 || player.isHeroType() == 2 && ConfigValue.SellHeroItemForPremium)
		{
	...
	public void getcloak(String[] var)
	{
		L2Player player = (L2Player) getSelf();
		if(player == null)
			return;
		int item = Integer.parseInt(var[0]);
		if(item < 32799 && item > 32802)
		{
			_log.info(player.getName() + " tried to obtain non hero item using hero cloak service. Ban him!");
			return;
		}

		if(player.isHeroType() == 0 || player.isHeroType() == 2 && ConfigValue.SellHeroItemForPremium)
		{
			for(String heroItem[] : HERO_CLOAK)
			{
				int itemId = Integer.parseInt(heroItem[0]);
				if(player.getInventory().getItemByItemId(itemId) != null || player.getWarehouse().countOf(itemId) > 0)
					return;
	...
	public void rendershop_c(String[] val)
	{
		L2Player player = (L2Player) getSelf();
		L2NpcInstance npc = getNpc();
		if(player == null || npc == null)
		...
		if(val[0].equalsIgnoreCase("list"))
		{
			htmltext = "<html><body><font color=\"LEVEL\">List of Hero Cloaks:</font><table border=0 width=270><tr><td>";
			for(int i = 0; i < HERO_CLOAK.length; i++)
			{
				htmltext += "<tr><td width=32 height=45 valign=top>";
				htmltext += "<img src=" + HERO_CLOAK[i][1] + " width=32 height=32></td>";
				htmltext += "<td valign=top>[<a action=\"bypass -h scripts_services.HeroItems:rendershop_c " + i + "\">" + HERO_CLOAK[i][2] + "</a>]<br1>";
				htmltext += "Type: " + HERO_CLOAK[i][3];
				htmltext += "</td></tr>";
			}
			htmltext += "</table>";
		}
		else if(Integer.parseInt(val[0]) >= 0 && Integer.parseInt(val[0]) <= HERO_CLOAK.length)
		{
			int itemIndex = Integer.parseInt(val[0]);
			...
			htmltext += "<table border=0 width=240>";
			htmltext += "<tr><td width=32 height=45 valign=top>";
			htmltext += "<img src=" + HERO_CLOAK[itemIndex][1] + " width=32 height=32></td>";
			htmltext += "<td valign=top>[<a action=\"bypass -h scripts_services.HeroItems:getcloak " + HERO_CLOAK[itemIndex][0] + "\" msg=\"" + msgId + "\">" + HERO_CLOAK[itemIndex][2] + "</a>]<br1>";
			htmltext += "Type: " + HERO_CLOAK[itemIndex][3] + "<br1>";
			htmltext += "</td></tr></table>";
			htmltext += "<font color=\"B09878\">" + HERO_CLOAK[itemIndex][3] + "</font>";
			htmltext += "</td></tr></table><br>";
			htmltext += "<img src=\"L2UI.SquareWhite\" width=270 height=1><br><br>";
			htmltext += "<CENTER><button value=Back action=\"bypass -h scripts_services.HeroItems:rendershop_c list\" width=40 height=15 back=L2UI_CT1.Button_DF fore=L2UI_CT1.Button_DF></CENTER>";
	...
	public String getcir()
	{
		...
		if(player == null)
			return null;
		if(player.getPlayer().isHeroType() == 0 || player.getPlayer().isHeroType() == 2 && ConfigValue.SellHeroItemForPremium)
		{
